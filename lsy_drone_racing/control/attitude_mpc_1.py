...
class AttitudeMPC(Controller):
    ...
    def __init__(
        ...
        if self._use_pytorch_mppi:
            ...
            self.goal = obs["gates_pos"][0] #+ np.array([-0.1, 0.0, 0.0])  # initial goal
            self.mppi_dt = 0.08 #self._dt
            ...
            def running_cost(x, u):
                ...
                # --- 1. Strong quadratic distance penalty ---
                dist_to_goal = torch.norm(pos - goal_t, dim=-1)
                c_pos = 60.0 * dist_to_goal**2
                
                # --- 2. Large gate penetration reward ---
                gate_radius = 0.45
                penetration = torch.maximum(
                    torch.zeros_like(dist_to_goal),
                    gate_radius - dist_to_goal
                )
                c_gate_bonus = -1500.0 * penetration
                
                # --- 3. Speed management ---
                speed = torch.norm(vel, dim=-1)
                close_to_gate = dist_to_goal < 0.55
                c_vel = torch.where(
                    close_to_gate,
                    12.0 * speed**2,  # Slow near gates
                    0.08 * (speed - 1.3)**2  # Prefer 1.3 m/s cruise
                )
                
                # --- 4. Light control penalty ---
                c_u = 0.004 * torch.sum(u ** 2, dim=-1)

                total_cost = c_pos + c_gate_bonus + c_vel + c_u
                return total_cost

            H = 60 #self._N
            K = 4048  # samples; tune for performance
            noise_sigma_matrix = 0.3 * torch.eye(3)  # Reduced from 0.4 for less noise
            ...
            self.mppi = MPPI(
                ...
                horizon=H,
                lambda_=1.0,
                u_min=torch.tensor([-2.0], dtype=torch.double, device=device),  # Reduced from -3.0
                u_max=torch.tensor([ 2.0], dtype=torch.double, device=device),  # Reduced from 3.0
                device=device,
            )
    ...
    def compute_control(
        ...
        if getattr(self, "_use_pytorch_mppi", False):
            # -------------------------------------------------
            # WARM-START MPPI: inject last optimal sequence
            # -------------------------------------------------
            if hasattr(self, "_prev_u") and self._prev_u is not None:
                # Make sure it's on the right device + dtype
                self.mppi.U = self._prev_u.to(self.mppi.U.device).clone()
            ...
            try:
                ...
                # 1) Get gate position from observation
                gate_pos = obs["gates_pos"][target_gate_idx]

                # 2) Compute gate normal using SAME METHOD as other controller
                #    - orientation from quaternion
                #    - pitch-locking
                rot = R.from_quat(obs["gates_quat"][target_gate_idx])
                det_euler = rot.as_euler("xyz")

                # Use nominal pitch if available in self._gates (same logic as detection controller)
                if hasattr(self, "_gates"):  
                    nom_pitch = self._gates[target_gate_idx]["nominal_rpy"][1]
                    pitch_lock_thresh = getattr(self, "_pitch_lock_thresh", np.deg2rad(7.5))

                    # Lock pitch if close (prevents vertical gate flips)
                    if abs(det_euler[1] - nom_pitch) < pitch_lock_thresh:
                        det_euler[1] = nom_pitch
                        rot = R.from_euler("xyz", det_euler)

                # 3) Gate normal = gate rotation matrix first column (x-axis)
                gate_normal = rot.as_matrix()[:, 0]
                gate_normal = gate_normal / np.linalg.norm(gate_normal)

                # 4) Shift goal forward by +10 cm along normal
                shift = 0 #0.10  # meters
                shifted_goal = gate_pos + shift * gate_normal

                # 5) Store and debug-print
            ...
            self._prev_u = self.mppi.U.clone()   # save for next warm-start
