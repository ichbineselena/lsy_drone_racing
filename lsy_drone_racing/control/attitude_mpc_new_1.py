...
class AttitudeMPC(Controller):
    ...
    def compute_control(
        ...
        # Extract obstacles (Nx3 array)
        obstacles = obs.get("obstacles_pos", None)
        if obstacles is not None and len(obstacles) > 0:
            # Parameters for avoidance
            safe_dist = 0.1   # meters; start repelling within this range
            w_repulse = 0.05  # tuning factor for repulsive field strength

            # Get current predicted trajectory positions
            ref_pos_seq = self._waypoints_pos[i : i + self._N].copy()
            yref = np.zeros((self._N, self._ny))
            for k in range(ref_pos_seq.shape[0]):
                pos = ref_pos_seq[k]
                repulse = np.zeros(3)
                for obs_pos in obstacles:
                    diff = pos - obs_pos
                    dist = np.linalg.norm(diff)
                    if dist < safe_dist and dist > 1e-3:
                        # Repulsive vector inversely proportional to squared distance
                        repulse += w_repulse * (diff / (dist**3)) * (safe_dist - dist)
                ref_pos_seq[k] += repulse  # shift waypoint away

            # Use adjusted positions as reference
            yref[:, 0:3] = ref_pos_seq
        else:
